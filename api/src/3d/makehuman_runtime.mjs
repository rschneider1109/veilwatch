...
export class MakeHumanRuntime {
  ...
  getAnimationTranslationScale(){
    return Number(this.animationTranslationScale||BASE_SCALE);
  }
  ...
  getBoneLocalBodyBounds(name,minWeight=.05){
    ...
    for(let vi=0;vi<=end;vi++){
      let weight=0;
      for(const row of this.vertexInfluences?.[vi]||[]) if(row[0]===boneIndex){ weight=Number(row[1])||0; break; }
      if(weight<minWeight) continue;
      const i=vi*3;
      p.set(this.transformed[i],this.transformed[i+1],this.transformed[i+2]);
      p.applyMatrix4(this.root.matrixWorld).applyMatrix4(inv);
      box.expandByPoint(p); count++;
    }
    if(!count || box.isEmpty()) return null;
    return {min:box.min.clone(),max:box.max.clone(),size:box.getSize(new THREE.Vector3()),center:box.getCenter(new THREE.Vector3()),count};
  }
